       ...
       for(my $i=1;$i<=$k;$i++){
             ...
             foreach my $oo(@arrs){
                 ...
                 if($allele =~ /DRB1\*14:01/){
                          system("bcftools  mpileup -r HLA_DRB1:9519-9519 -a DP,SP -Ov -f $db/hla.ref.extend.fa $dir/$sample.realign.sort.bam -o $workdir/snp.vcf");
                 ...
                 if($allele =~ /C\*07:01/ && $wxs eq "exon"){
                          system("bcftools  mpileup -r HLA_C:4061-4061 -a DP,SP -Ov -f $db/hla.ref.extend.fa $dir/$sample.realign.sort.bam -o $workdir/snp.vcf");
                          open TE, "$workdir/snp.vcf" or die "$!\n";
                          while(<TE>){
                                 chomp;
                                 next if(/^#/);
